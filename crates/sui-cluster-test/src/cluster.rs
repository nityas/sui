...
use sui_config::{
    sui_cluster_test_config_dir, sui_config_dir, SUI_KEYSTORE_FILENAME, SUI_NETWORK_CONFIG,
};
...
impl Cluster for LocalNewCluster {
    async fn start(
        ...
        let (network_config_path, provided_config) = if genesis_config.is_some() {
            // This assumes the usage of the same network config used in sui start/genesis
            (Some(sui_config_dir()?.join(SUI_NETWORK_CONFIG)), true)
        } else {
            (None, false)
        };
